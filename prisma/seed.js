...
/**
 * 与 prisma/seed.ts 等价的一份种子：容器 entrypoint.sh 用纯 node 执行本文件
 * （运行时无 ts-node），本地 `prisma db seed` 用 ts-node 执行 seed.ts。
 * 改动务必同步两份。
 *
 * 幂等：管理员已存在时保留其现有口令（不覆盖、不重置），且不会打印任何口令，
 * 避免重跑时输出一个其实没写进库、登录即用不了的误导性口令。
 * 明文口令不再硬编码；未设 ADMIN_INITIAL_PASSWORD 则随机生成并只在真正新建时打印一次。
 */
